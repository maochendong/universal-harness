...
- **本地 Dashboard**：`harness serve [--port <port>]` 只监听 loopback，提供 Overview、Graph、Impact、Iteration、Evidence、Findings、Live 和 Approvals 八个视图。Approvals 从 Ledger 恢复当前待审批队列，不依赖浏览器是否收到实时事件；随机一次性 URL token 交换为 HttpOnly session，写操作要求同源 Origin、session CSRF、actor 与 expected digest，并复用原有 Approval/Resume/Finding 服务。
