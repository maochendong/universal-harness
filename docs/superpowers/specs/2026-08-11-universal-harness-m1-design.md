...
`harness audit` 检查 traceability coverage、stale knowledge、互相冲突的已接受 Constraint、orphan Node、missing verification、未提升的高风险 Improvement、ContextBundle source health，关键设计/决策文档覆盖度（`missing_design_artifact`：存在 ExecutionPlan 后，design、API contract、data design、frontend design（仅有前端信号时）和 decision record 五类领域必须有已接受的文档或 Decision 节点，缺失为 warning），以及 analyze 式覆盖规则：`task_orphan`（blocking：Task 没有到任何 Requirement 的 IMPLEMENTS 边，与 traceability_gap 互为反向不重复）、`api_contract_coverage`（warning：已接受合同文档提取出的 endpoint/heading 条目无任何 Task 引用；无合同文档时静默，由 missing_design_artifact 发言）、`task_stale`（blocking：已 accepted 的 Task 既无 accepted EvaluationCase EVALUATES 裁决、也无 fresh 通过的任务级质量记录；与 missing_verification 作用节点集不相交）。plan 相位为每个 Task 提交到其 expected_outputs Requirement 的 IMPLEMENTS 边，作为上述链路的事实源。Audit Finding 进入与 Test/Review Failure 相同的 Finding/ImpactSet 流程。迭代完成快照时编排器先对工作区做增量文档重扫（adopt 后新增的文档文件补入图，含合同条目提取），再自动重跑同一套审计（以 fresh 质量记录集作为 task_stale 的排除上下文），把缺口按内容派生 id 幂等地提交为 Finding 节点——blocking 缺口附带到当前迭代的 BLOCKS 边并出现在 `harness status` 的 blockers / next_action，非阻塞缺口列为 status 的 warnings——无需手动 `harness audit`；缺口消失时对应 Finding 被 supersede。
