...
describe("protocol registry", () => {
  it("registers 1.0.0 as stable and 1.1.0 as in-development on the same major", () => {
    expect(PROTOCOL_REGISTRY.map((entry) => entry.version)).toEqual(["1.0.0", "1.1.0"]);
  ...
  it("fails closed on unknown or malformed protocol versions", () => {
    ...
    expect(isKnownProtocol("1.2.0")).toBe(false);
    expect(isKnownProtocol("2.0.0")).toBe(false);
    expect(isKnownProtocol("not-semver")).toBe(false);
    expect(() => assertKnownProtocol("1.2.0")).toThrow(/unknown protocol version/i);
