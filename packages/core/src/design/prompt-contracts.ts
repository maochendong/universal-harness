...
export const DESIGN_REVIEW_PROMPT_CONTRACT: PromptContract = definePromptContract({
  ...
  domain_rubric: {
    segment_id: "domain-rubric",
    text: "Every finding must name a severity and category, the affected asset or criterion, cited source references, the observed problem, a recommended revision and a suggested verification. Every source_ref must copy a ref/digest pair verbatim from the bundle_sources input — a citation that matches no bundled source exactly invalidates the review. Every finding must carry at least one affected target drawn from the proposal itself: affected_asset_id must be copied verbatim from the node_id of a node_changes or reused_assets member of the proposal content, and affected_criterion_id must be copied verbatim from an acceptance_criterion_id in the proposal content's coverage test_strategy_coverage — a requirement id, a test node id or any invented identifier is never a valid affected target and invalidates the review. Assess the coverage of exactly the must-change requirements: exactly one coverage_assessment entry per must-change requirement id and no entry for any other id. Report residual risks honestly. Return blocked only with at least one critical finding, and accept_recommended only when no critical finding remains.",
  },
  profile_overlays: {
    lite: {
      segment_id: "profile-lite",
