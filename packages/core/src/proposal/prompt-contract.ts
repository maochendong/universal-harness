...
export const PRD_PROPOSAL_PROMPT_CONTRACT: PromptContract = definePromptContract({
  ...
  domain_rubric: {
    ...
    text: "Every requirement is atomic, observable and testable; every must-change requirement carries an acceptance criterion with a test-first example. One criterion covers exactly one independently verdictable outcome: its observable_outcome is a single clause on a single line — no semicolons, no line breaks, no 'and then'/'并且'/'以及' joins; anything compound becomes separate criteria. The draft's intent must restate the operator intent verbatim, character for character, and copy the intent digest verbatim from the session binding — never paraphrase, translate, normalize or recompute either. Every entity's source_bindings must be verifiable verbatim copies: the intent binding is exactly source_kind intent with source_id intent and the session intent digest; a project_context binding copies its locator as source_id and the source_digest from the session binding's proposal_bundle_sources; a clarification_answer binding copies the answer id and record digest from the accepted answers — never invent a binding or a digest. The draft must link requirements and criteria by exact key: every requirement's acceptance_criterion_ids reference the draft_key of criteria you declare, every criterion's requirement_id references the draft_key of its requirement, and no requirement may end up with zero criteria. A draft always contains at least one requirement; when the intent cannot ground any requirement, return clarification questions instead of an empty draft. When the inputs leave a material decision ambiguous, return clarification questions instead of guessing. Never invent scope that is not grounded in the provided inputs.",
  },
  profile_overlays: {
    lite: {
