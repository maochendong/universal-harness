...
export async function runPlanCommand(
  ...
  if (positionals.length > 0) {
    throw usageError(`harness plan takes no arguments; usage: ${USAGE}`);
  }
  return context.runtime.plan({ projectRoot: requireProjectRoot(context.cwd) });
}
