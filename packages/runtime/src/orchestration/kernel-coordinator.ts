...
import { type TaskVerdictRecord } from "../evaluation/task-verdict.js";
...
import { buildSnapshot, snapshotCompletionBlockers } from "../snapshot/builder.js";
...
import { PHASE_CHECKPOINT_BOUNDARY, type OrchestrationPhase } from "./phases.js";
import {
...
export async function commitVerifiedSourceTree(
  ctx: PipelineContext,
...
export function sourceRootOf(ctx: PipelineContext): string {
...
/** Live observation hook; failures never affect outcomes. */
export function observe(_ctx: PipelineContext, action: () => unknown): void {
  try {
    action();
  } catch {
...
export function acceptedPrdForOperation(ctx: PipelineContext) {
...
export async function ensureInitialCapabilityPlan(
  ctx: PipelineContext,
...
export function activateModulesFromCapabilityPlan(
...
export async function finalizeCapabilityPlan(
  ctx: PipelineContext,
...
export function loadVerifyArtifact(
  deps: OrchestratorDependencies,
  iterationId: string,
  bindings: VerifyPhaseArtifact["bindings"],
...
export async function commitIterationNode(
  ctx: PipelineContext,
...
export async function phaseCapture(ctx: PipelineContext): Promise<PhaseStep> {
  const { deps } = ctx;
...
export function loadTaskTddContract(
  ctx: PipelineContext,
  taskId: string,
...
export function taskTddVerdictInput(
  ctx: PipelineContext,
...
export async function phaseContext(ctx: PipelineContext): Promise<PhaseStep> {
  const { deps } = ctx;
  const plan = ctx.plan ?? loadPlan(ctx);
  if (plan === undefined) {
 ...
 */
export async function commitTaskVerdict(
  ctx: PipelineContext,
  verdict: TaskVerdictRecord,
): Promise<void> {
  const { deps } = ctx;
...
export async function phaseExecute(ctx: PipelineContext): Promise<PhaseStep> {
  const { deps } = ctx;
  const plan = ctx.plan ?? loadPlan(ctx);
  if (plan === undefined)
...
export function verifyBindings(ctx: PipelineContext): VerifyPhaseArtifact["bindings"] {
 ...
 */
export function storedEvidenceMaterials(
  deps: OrchestratorDependencies,
...
export async function commitEvidenceNodes(
  ctx: PipelineContext,
 ...
 */
export async function commitScannedDocumentation(ctx: PipelineContext): Promise<void> {
  const { deps } = ctx;
  const graph = materializeProjectGraph(deps.projectRoot);
